...
te = twenty_all[twenty_all$Position == "TE", ]
...
for(j in 1:100) {
  set.seed(j)
  split = createDataPartition(te$points_average, p = 0.8, list = FALSE)
  train = te[split, ]
  test = te[-split, ]
  for(i in 2:15) {
...
te_predict = predict(best_sp, c(1:200))$y

te_deriv = predict(best_sp, c(1:200), deriv = TRUE)$y
