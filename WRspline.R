...
wr = twenty_all[twenty_all$Position == "WR", ]
...
for(j in 1:100) {
  set.seed(j)
  split = createDataPartition(wr$points_average, p = 0.8, list = FALSE)
  train = wr[split, ]
  test = wr[-split, ]
  for(i in 2:15) {
...
wr_predict = predict(best_sp, c(1:200))$y

wr_deriv = predict(best_sp, c(1:200), deriv = TRUE)$y