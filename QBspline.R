...
qb = twenty_all[twenty_all$Position == "QB", ]
...
for(j in 1:100) {
  set.seed(j)
  split = createDataPartition(qb$points_average, p = 0.8, list = FALSE)
  train = qb[split, ]
  test = qb[-split, ]
  for(i in 2:15) {
...
qb_predict = predict(best_sp, c(1:200))$y

qb_deriv = predict(best_sp, c(1:200), deriv = TRUE)$y